...
import axios from "axios"; // to send data to MongoDB
import { useNavigate } from "react-router-dom";
import Dashboard from "./Dashboard"; // assuming Dashboard is your full-screen component
...
const Questionnaire = () => {
    ...
    const handleSubmit = async () => {
        try {
            await axios.post("http://localhost:5000/api/questionnaire", answers);
            console.log("Questionnaire submitted successfully");

            // Show the full-screen dashboard pop-up
            ...
            >
                        ...
                        <div key={index} className="mb-6">
                            ...
                            {q.type === "input" && (
                                <input
                                    ...
                                    className="bg-gray-700 border-gray-600 text-white p-2 w-full"
                                />
                            )}
                            {q.type === "radio" && (
                                    ...
                                    {q.options?.map((option) => (
                                        <div key={option} className="flex items-center space-x-2">
                                            <input
                                                ...
                                                onChange={() => handleInputChange(q.key, option)}
                                            />
                                            <label>{option}</label>
                                        </div>
                                    ))}
                ...
                <div className="flex justify-between mt-8">
                    <button
                        onClick={handleBack}
                        disabled={step === 0}
                        className="flex items-center bg-gray-600 text-white p-2 rounded"
                    >
                        Back
                    </button>
                    {step === questions.length - 1 ? (
                        <button
                            ...
                            className="flex items-center bg-green-600 hover:bg-green-700 text-white p-2 rounded"
                        >
                            Submit
                        </button>
                    ) : (
                        <button
                            onClick={handleNext}
                            className="flex items-center bg-purple-600 hover:bg-purple-700 text-white p-2 rounded"
                        >
                            Next
                        </button>
                    )}
